# Third-Party Notices

Processing 5 - Creative Coding Station incorporates the following third-party
software. Each component remains the property of its respective copyright
holders and is governed by its own license, reproduced or referenced below.

---

## p5.js

Copyright (c) 2026 Processing Foundation
License: GNU Lesser General Public License v2.1 (LGPL-2.1)
https://github.com/processing/p5.js

## p5.sound.js (p5.js Sound addon, v1.0.1)

Copyright (c) 2014-2026 Jason Sigal, Yotam Mann, and p5.js Sound contributors
License: GNU Lesser General Public License v2.1 (LGPL-2.1)
https://github.com/processing/p5.js-sound

## WPF-UI (lepoco/wpfui)

Copyright (c) 2021-2025 Leszek Pomianowski and WPF UI Contributors
License: MIT License
https://github.com/lepoco/wpfui

## AvalonEdit (ICSharpCode.AvalonEdit)

Copyright (c) AvalonEdit Contributors
License: MIT License
https://github.com/icsharpcode/AvalonEdit

## AvalonDock (Dirkster99/AvalonDock)

Copyright (c) Xceed Software Inc. and Dirkster99/AvalonDock Contributors
License: Microsoft Public License (Ms-PL)
https://github.com/Dirkster99/AvalonDock

## AvalonDock.Themes.WPFUI

License: MIT License
https://github.com/qian-o/AvalonDock.Themes.WPFUI

## Microsoft Edge WebView2 SDK

Copyright (C) Microsoft Corporation. All rights reserved.
License: WebView2 SDK License (BSD-style redistribution terms)
https://developer.microsoft.com/microsoft-edge/webview2/

## FFmpegCore

License: MIT License
https://github.com/rosenbjerg/FFMpegCore

## FFmpeg (embedded binary, ffmpeg.exe, build n7.1.5)

Copyright (c) FFmpeg developers
License: GNU Lesser General Public License v3 (LGPL-3.0)
Build: BtbN/FFmpeg-Builds "win64-lgpl" (static, LGPL-only codec set — notably
excludes the GPL-only libx264/libx265; H.264 encoding uses the BSD-licensed
Cisco OpenH264 encoder instead, matching the LGPL build's license terms)
https://ffmpeg.org/legal.html
https://github.com/BtbN/FFmpeg-Builds

## OpenH264 (Cisco, linked by the embedded ffmpeg.exe for H.264/MP4 export)

Copyright (c) Cisco Systems, Inc.
License: BSD 2-Clause License
https://github.com/cisco/openh264

## SixLabors.ImageSharp

Copyright (c) Six Labors
License: Six Labors Split License v1.0 (free for open-source and for
organizations under the license's revenue threshold; a commercial license
is required above that threshold)
https://github.com/SixLabors/ImageSharp/blob/main/LICENSE

## CommunityToolkit.Mvvm

Copyright (c) .NET Foundation and Contributors
License: MIT License
https://github.com/CommunityToolkit/dotnet

## Serilog (core, Sinks.File, Extensions.Logging)

Copyright (c) Serilog Contributors
License: Apache License 2.0
https://serilog.net/

## Microsoft.Extensions.* (DependencyInjection, Logging)

Copyright (c) .NET Foundation and Contributors
License: MIT License
https://github.com/dotnet/runtime
